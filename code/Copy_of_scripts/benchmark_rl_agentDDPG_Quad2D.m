function completed = benchmark_rl_agentDDPG_Quad2D()
% benchmark_neuralNetwork_rl_agentDDPG_Quad2D - Example for set-based
...
%
% Syntax:
%    res = benchmark_neuralNetwork_rl_agentDDPG_Quad2D()
%
% Inputs:
%    -
%
...
% System Dynamics ---------------------------------------------------------
sys = nonlinearSys(@Quad2D_Dynamics);
...
% Env settings
options.rl.env.x0 = interval([-1;0;-1;0;0;0],[1;0;1;0;0;0]);
...
env = ctrlEnvironment(sys,@aux_rewardFun_Quadrocopter2D,@aux_collisionCheck_Quadrocopter2D,options);
...
evaluateAgentsStatistically(agents,env,0:0.05:0.1,'inf');
...
function reward = aux_rewardFun_Quadrocopter2D(x)
if isnumeric(x)
    reward = -(abs(x(end,1))+0.01*abs(x(end,2))+abs(x(end,3))+0.01*abs(x(end,4)));
else
    w = [1,0.01,1,0.01,0,0,0,0];
    reward = -max(abs(supportFunc(x.timePoint.set{end},w,'lower')),abs(supportFunc(x.timePoint.set{end},w)));
end
end

function collisionBool = aux_collisionCheck_Quadrocopter2D(x)
if isa(x,'numeric')
    if all(abs(x(:,1:4))<0.05,"all")
        collisionBool = true;
    else
        collisionBool = false;
    end
...
function f = Quad2D_Dynamics(x,u)
    % Parameters
    m = 0.027;
    g = 9.81;
    Iy = 1.4e-5;
    ...
    % dynamics
    f = [x(2);
    sin(x(5))*((1+.1*u(1))*g/2+(1+.1*u(2))*g/2);
    x(4);
    cos(x(5))*((1+.1*u(1))*g/2+(1+.1*u(2))*g/2)-g;
    x(6);
    l*((1+.1*u(2))*m*g/2-(1+.1*u(1))*g*m/2)/Iy/sqrt(2);];
end
